    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-elevated pb-4 sm:pb-6 gap-3 sm:gap-4">
        <div className="min-w-0">
          <h2 className="text-xl sm:text-2xl font-extrabold text-accent mb-1">Analysis Results</h2>
          <p className="text-sm sm:text-base text-text-secondary">Detected Sentiment: <span className="font-semibold text-text-primary">{data.sentiment}</span></p>
        </div>
        <div className={`px-4 sm:px-6 py-1.5 sm:py-2 rounded-full border-2 font-bold text-sm sm:text-lg shadow-sm shrink-0 ${getBadgeColor(data.urgency)}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4">
          {Object.entries(data.drafts).map(([key, draft]) => (
            <Link key={key} href={`/result/draft/${key}`} className="block p-3 sm:p-4 md:p-5 border-2 border-elevated rounded-xl hover:border-accent cursor-pointer transition-colors bg-elevated shadow-sm">
